...
namespace Native.Csharp.App.Event
{
	public class Event_GroupMessage : IEvent_GroupMessage
	{
		...
		public void ReceiveGroupMessage (object sender, GroupMessageEventArgs e)
		{
            ...
            try
            {
                Regex regex = new Regex(@"^.(\S+) *(\d*)\r\n([\s\S]+)$");   //注意匹配1后面是“ *”，也就是任意个空格
                Match match = regex.Match(e.Msg);
                if (match.Success)
                {
                    string type = match.Groups[1].Value;
                    if (type=="表情"||type=="表情包")
                        type = Pic.GetRandomPic();

                    if (Pic.PicExist(type))
                    {
                        Common.CqApi.AddLoger(Sdk.Cqp.Enum.LogerLevel.Debug, "生成表情", type);
                        Pic pic = new Pic(type);

                        int.TryParse(match.Groups[2].Value, out int size);
                        string content = match.Groups[3].Value;
                        pic.Make(content, size);

                        string filename = pic.Save(e.FromGroup);
